    ...
    int32_t *adc_T)
{
    ...
    i2c_read(dev->address, BMP280_REG_DATA, raw, 6);
    ...
    uint8_t addr)
{
    ...
    if (i2c_read(addr, BMP280_REG_ID, &id, 1) != ESP_OK)
        return false;

    if (id != 0x58)
        return false;

    i2c_write(addr, BMP280_REG_CTRL, 0x27);
    i2c_write(addr, BMP280_REG_CONFIG, 0x10);
...
float bmp280_get_pressure(bmp280_t *dev)
{
    int32_t adc_P, adc_T;

    read_raw(dev, &adc_P, &adc_T);

    compensate_temp(dev, adc_T);

    return compensate_pressure(dev, adc_P);
}

void bmp280_calibrate_ground(bmp280_t *dev)
{
    ...
    dev->base_pressure = sum / samples;
}

float bmp280_get_altitude(bmp280_t *dev)
{
    float pressure = bmp280_get_pressure(dev);

    float altitude =
        44330.0f *
        (1.0f - powf(pressure / dev->base_pressure, 0.1903f));
